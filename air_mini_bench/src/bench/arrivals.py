...
def _microburst(n: int, rng: random.Random, wave_min: int, wave_max: int, gap_range: tuple[int, int]) -> list[int]:
    ts: list[int] = [0]
    i = 1
    while i < n:
...
def _large_burst(n: int, rng: random.Random, batch_min: int, batch_max: int, gap_range: tuple[int, int]) -> list[int]:
    return _microburst(n, rng, batch_min, batch_max, gap_range)
...
def _lc_spread_poisson(n: int, lc_positions: set[int], rng: random.Random, mean_ms: float) -> list[int]:
...
def build_session_clustered_timestamps(
    ...
    """Theo thứ tự replay: trong cùng session timestamp gần nhau, luôn không giảm."""
    ts_map: dict[str, int] = {}
    t = 0
...
# Legacy names (timestamp-only replays)
LEGACY_ARRIVAL_ALIASES = {
    "contest": "steady_poisson",
    "fast": "fast_queue",
    "burst": "large_burst",
    "overlap": "microburst",
}
