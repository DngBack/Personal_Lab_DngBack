...
def enforce_prompt_tokens(text: str, max_tokens: int) -> tuple[str, int]:
    unit = " word"
    cur = estimate_token_count(text)
    if cur < max_tokens:
        text = text + unit * (max_tokens - cur)
    n = estimate_token_count(text)
    while n > max_tokens and len(text) > 0:
        text = text[: max(1, len(text) - tokens_to_chars(max(16, n - max_tokens)))]
        n = estimate_token_count(text)
    return text, n
