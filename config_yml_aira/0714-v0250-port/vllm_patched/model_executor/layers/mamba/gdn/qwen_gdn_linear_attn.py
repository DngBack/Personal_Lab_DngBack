    ...
    * Blackwell (SM10.x) with ``head_k_dim == 128``;
    ...
    if (
        ...
        try:
            ...
            supports_flashqla = _importlib_util.find_spec("flash_qla") is not None
        except Exception as exc:  # noqa: BLE001
            logger.warning_once("AIR FlashQLA probe failed: %s", exc)
...
def air_chunk_gated_delta_rule(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    # AIR_FLASHQLA_V1: thin trampoline into /app/air_flashqla_runtime; the stock
    # flashinfer kernel below doubles as the in-flight fallback.
    import sys
    if "/app" not in sys.path:
        sys.path.insert(0, "/app")
    from air_flashqla_runtime import run_air_flashqla
    return run_air_flashqla(
        q=q,
        k=k,
        v=v,
        g=g,
        ...
        fallback=fi_chunk_gated_delta_rule,
        logger=logger,
    )


# P26-GDN-SCANFUSE: env-gated ports of vLLM PR#45717 (GDN in-proj horizontal
# fusion + direct scan-output write), applied on top of the existing
# AIR_FLASHQLA_V1 / P21-GDN-MINIFUSE patches. See module docstring for the
# per-flag rationale. Both default OFF (byte-identical to r8 when unset).
import os as _p26_os

_P26_FUSE_IN_PROJ = _p26_os.getenv("VLLM_GDN_FUSE_IN_PROJ", "0") == "1"
_P26_DIRECT_SCAN_OUTPUT = _p26_os.getenv("VLLM_GDN_DIRECT_SCAN_OUTPUT", "0") == "1"
...
class ChunkGatedDeltaRule(CustomOp):
    ...
    def forward_cuda(
        ...
        # P26-GDN-SCANFUSE: when the caller buffer matches the scan output
        # exactly, let FlashInfer write into it in-kernel and skip the bridge
        # copy; any mismatch falls back to the original copy path (fail-safe).
        fi_direct_out = None
        if (
            _P26_DIRECT_SCAN_OUTPUT
            and core_attn_out is not None
            and core_attn_out.dim() == 3
            and core_attn_out.is_contiguous()
            and core_attn_out.dtype == v.dtype
            and core_attn_out.numel() == v.numel()
        ):
            fi_direct_out = core_attn_out
        o, final_state = fi_chunk_gated_delta_rule(
            ...
            cu_seqlens=cu_seqlens,
            use_qk_l2norm_in_kernel=use_qk_l2norm_in_kernel,
            g_already_exp=g_already_exp,
            output=fi_direct_out,
        )
        if core_attn_out is not None and fi_direct_out is None:
...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        # P26-GDN-SCANFUSE: lazy horizontal in_proj fusion state. Tri-state:
        # None = not attempted yet (first forward_cuda tries), False = off or
        # preconditions failed (fail-safe), True = fused fast path on.
        self._p26_fuse_in_proj: bool | None = None
        self._p26_in_proj_fused_weight: torch.Tensor | None = None
        self._p26_fused_qkvz_size = 0
        ...
        if self.gdn_prefill_backend == "flashinfer":
            logger.info_once(
                "P21-GDN-MINIFUSE active: fused_post_conv_prep emits exp(g) "
                "for FlashInfer and fi wrapper skips torch.exp(g)."
            )
    ...
    def _p26_fuse_in_proj_once(self) -> None:
        """P26-GDN-SCANFUSE: build the concatenated in_proj weight lazily.

        Runs on the first forward_cuda call (the profiling forward, before any
        CUDA-graph capture), so the fused weight is a constant tensor by
        capture time. Only fuses plain unquantized bf16/fp16 CUDA weights
        without bias; anything else (e.g. our --quantization=fp8 in_proj
        layers) keeps the two-GEMM path.
        """
        self._p26_fuse_in_proj = False
        if not _P26_FUSE_IN_PROJ:
            return

        def _disable(reason: str) -> None:
            logger.info_once(f"P26-GDN-SCANFUSE fuse-in-proj DISABLED: {reason}")

        proj_qkvz = getattr(self, "in_proj_qkvz", None)
        proj_ba = getattr(self, "in_proj_ba", None)
        if proj_qkvz is None or proj_ba is None:
            return _disable("missing in_proj modules (LoRA split path?)")
        if getattr(proj_qkvz, "bias", None) is not None:
            return _disable("in_proj_qkvz has bias")
        if getattr(proj_ba, "bias", None) is not None:
            return _disable("in_proj_ba has bias")
        w_qkvz = getattr(proj_qkvz, "weight", None)
        w_ba = getattr(proj_ba, "weight", None)
        if w_qkvz is None or w_ba is None:
            return _disable("missing weight tensors")
        if w_qkvz.ndim != 2 or w_ba.ndim != 2:
            return _disable(f"ndim {w_qkvz.ndim}/{w_ba.ndim} != 2")
        if w_qkvz.dtype not in (torch.bfloat16, torch.float16):
            return _disable(f"weight dtype {w_qkvz.dtype} (quantized?)")
        if w_qkvz.dtype != w_ba.dtype or w_qkvz.device != w_ba.device:
            return _disable(f"dtype/device mismatch {w_qkvz.dtype}/{w_ba.dtype}")
        if w_qkvz.shape[1] != w_ba.shape[1]:
            return _disable(f"K mismatch {w_qkvz.shape} vs {w_ba.shape}")
        if not (w_qkvz.is_cuda and w_ba.is_cuda):
            return _disable("weights not CUDA-resident")

        n_qkvz = w_qkvz.shape[0]
        fused = torch.cat([w_qkvz.data, w_ba.data], dim=0).contiguous()
        # Re-point the original parameters at views into the fused buffer so
        # the eager two-GEMM path stays available and weight memory is not
        # duplicated once the old storages are freed.
        proj_qkvz.weight.data = fused[:n_qkvz]
        proj_ba.weight.data = fused[n_qkvz:]
        self._p26_in_proj_fused_weight = fused
        self._p26_fused_qkvz_size = n_qkvz
        self._p26_fuse_in_proj = True
        logger.info_once(
            f"P26-GDN-SCANFUSE fuse-in-proj active: single GEMM "
            f"N={fused.shape[0]} (qkvz {n_qkvz} + ba {w_ba.shape[0]})."
        )
    ...
    def forward_cuda(
        ...
        # ============================================================
        # P26-GDN-SCANFUSE: one fused GEMM over [qkvz|ba] when enabled.
        if self._p26_fuse_in_proj is None:
            self._p26_fuse_in_proj_once()
        if self._p26_fuse_in_proj:
            fused_out = torch.nn.functional.linear(
                hidden_states, self._p26_in_proj_fused_weight
            )
            mixed_qkvz = fused_out[..., : self._p26_fused_qkvz_size]
            ba = fused_out[..., self._p26_fused_qkvz_size :]
            if self.gqa_interleaved_layout:
                # fix_query_key_value_ordering() uses .view(), which needs
                # contiguous inputs; the non-interleaved (Qwen3.5) path
                # consumes the strided slices via split/reshape/contiguous.
    ...
    def _warmup_prefill_kernels(self, qkv_or_qkvz: torch.Tensor, v_dim: int) -> None:
        ...
        q, k, v, g, beta = fused_post_conv_prep(
            ...
            apply_l2norm=True,
            output_g_exp=(self.gdn_prefill_backend == "flashinfer"),
        )
        q = q.unsqueeze(0)
        k = k.unsqueeze(0)
        v = v.unsqueeze(0)
        ...
        try:
            self.chunk_gated_delta_rule(
                ...
                g_already_exp=(self.gdn_prefill_backend == "flashinfer"),
            )
        except Exception:
            logger.warning(
    ...
    def _forward_core(
        ...
        # P26-GDN-SCANFUSE: thread the output buffer into the FlashInfer
        # chunk scan (kernel-level output=) for the no-spec, non-split prefill
        # branch so the bridge copy below becomes redundant. Gated to the
        # flashinfer backend; every other path keeps its copy.
        direct_scan_output = (
            _P26_DIRECT_SCAN_OUTPUT
        ...
        if direct_scan_output:
            logger.info_once(
                "P26-GDN-SCANFUSE direct-scan-output active: FlashInfer "
                "writes core_attn_out in-kernel; bridge copy elided."
            )
        ...
        if spec_sequence_masks is not None and core_attn_out_non_spec is not None:
            ...
            # P26-GDN-SCANFUSE: when direct_scan_output is on, FlashInfer
            # already wrote core_attn_out[:num_actual_tokens] in-kernel (or
            # forward_cuda's fail-safe copy filled it), so skip this copy.
