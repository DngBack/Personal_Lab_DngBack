...
if __name__ == "__main__":
    ...
    # P36-RUST-DISPATCH: the Rust frontend dispatch normally lives only
    # in the `vllm serve` CLI (cli/serve.py cmd). The grading system
    # requires this module as the container entrypoint, so mirror that
    # dispatch here. Inert unless VLLM_USE_RUST_FRONTEND=1.
    import vllm.envs as _p36_envs
    if _p36_envs.VLLM_RUST_FRONTEND_PATH:
        if getattr(args, "api_server_count", None) is None:
            args.api_server_count = 1
        if not hasattr(args, "headless"):
            args.headless = False
        # vllm-rs requires model_tag in --args-json; `vllm serve` gets it
        # from its positional arg, api_server has no such positional.
