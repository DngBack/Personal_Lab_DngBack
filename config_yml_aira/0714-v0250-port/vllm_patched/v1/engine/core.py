            ...
            while True:
                for input_socket, _ in poller.poll():
                    ...
                    if request_type == EngineCoreRequestType.ADD:
                        ...
                        # ENGPACE v2: wall-clock token-bucket pacing of ADD
                        # requests in the input IO thread (ENGPACE_RATE_MS>0
                        # to enable). Delays only subsequent ADD deliveries,
                        # never the engine busy loop — reproduces frontend
                        # bucket pacing semantics under any frontend.
                        if not hasattr(self, "_engpace_rate"):
                            import os as _os
                            self._engpace_rate = float(
                                _os.environ.get("ENGPACE_RATE_MS", "0") or "0")
                            self._engpace_burst = float(
                                _os.environ.get("ENGPACE_BURST_N", "8") or "8")
                            # v3 idle-gate: khi số request đang DECODE trong
                            # scheduler <= ngưỡng này thì bỏ pacing (engine
                            # rỗng nuốt burst prefill tốt hơn bị rải đều —
                            # dữ liệu P35: d4 unpaced 277ms < paced 360ms).
                            # -1 = tắt (giữ nguyên hành vi v2).
                            self._engpace_idle = int(
                                _os.environ.get("ENGPACE_IDLE_DECODES", "-1")
                                or "-1")
                            self._engpace_log = (
                                _os.environ.get("ENGPACE_LOG", "") == "1")
                            self._engpace_seq = 0
                            # v4 fill-mode: khi len(running) < FILL_RUN là
                            # đang lấp window (burst 40 tại t=0, engine rỗng
                            # nuốt tốt) — rải nhanh FILL_RATE_MS thay vì
                            # bucket. Telemetry P36: fill có run ≈ vị-trí
                            # 0→38, steady-state luôn run>=33. 0 = tắt.
                            self._engpace_fill_run = int(
                                _os.environ.get("ENGPACE_FILL_RUN", "0")
                                or "0")
                            self._engpace_fill_rate = float(
                                _os.environ.get("ENGPACE_FILL_RATE_MS", "0")
                                or "0")
                            self._engpace_fill_last = 0.0
                            # v5 latch: fill chi? arm khi quan sát run==0
                            # (chỉ xảy ra giữa các run/warmup boundary,
                            # không bao giờ giữa run) và tát vĩnh viẽn
                            # khi run >= FILL_RUN — tránh fill tự kích
                            # hoạt lại ở d6-d12 khi completions dừn cục
                            # làm running tụt (bài hơc FILL4-b 85.53).
                            self._engpace_fill_armed = False
                            self._engpace_tokens = self._engpace_burst
                            self._engpace_last = time.monotonic()
                            logger.info(
                                "ENGPACE-v2 active: rate_ms=%s burst_n=%s"
                                " idle_decodes=%s fill_run=%s"
                                " fill_rate_ms=%s (v5-latch)",
                                self._engpace_rate, self._engpace_burst,
                                self._engpace_idle, self._engpace_fill_run,
                                self._engpace_fill_rate)
                        if self._engpace_rate > 0:
                            _now = time.monotonic()
                            _run = len(self.scheduler.running)
                            _dec = -1
                            _wait = 0.0
                            if _run == 0:
                                self._engpace_fill_armed = True
                            elif _run >= self._engpace_fill_run:
                                self._engpace_fill_armed = False
                            _fill = (self._engpace_fill_run > 0
                                     and self._engpace_fill_armed
                                     and _run < self._engpace_fill_run)
                            if _fill:
                                _tgt = (self._engpace_fill_last
                                        + self._engpace_fill_rate / 1000.0)
                                if _now < _tgt:
                                    _wait = _tgt - _now
                                    time.sleep(_wait)
                                self._engpace_fill_last = time.monotonic()
                                # thoát fill không mang credit burst theo
                                self._engpace_tokens = 0.0
                                self._engpace_last = self._engpace_fill_last
                            else:
                                self._engpace_tokens = min(
                                    self._engpace_burst,
                                    self._engpace_tokens
                                    + (_now - self._engpace_last)
                                    * 1000.0 / self._engpace_rate,
                                )
                                self._engpace_last = _now
                                if (self._engpace_idle >= 0
                                        or self._engpace_log):
                                    # Đọc cross-thread chỉ để heuristic;
                                    # list() copy nguyên tử dưới GIL.
                                    try:
                                        _dec = sum(
                                            1 for _r in
                                            list(self.scheduler.running)
                                            if _r.num_computed_tokens
                                            >= _r.num_prompt_tokens)
                                    except Exception:
                                        _dec = -1
                                if (self._engpace_tokens < 1.0
                                        and 0 <= _dec <= self._engpace_idle):
                                    self._engpace_tokens = 1.0
                                if self._engpace_tokens < 1.0:
                                    _wait = (1.0 - self._engpace_tokens) \
                                        * self._engpace_rate / 1000.0
                                    time.sleep(_wait)
                                    self._engpace_last = time.monotonic()
                                    self._engpace_tokens = 1.0
                                self._engpace_tokens -= 1.0
                            if self._engpace_log:
                                self._engpace_seq += 1
                                logger.info(
                                    "ENGPACE add#%d t=%.3f dec=%d run=%d "
                                    "wait=%.1fms ptoks=%s fill=%d",
                                    self._engpace_seq, _now, _dec, _run,
                                    _wait * 1000.0,
                                    getattr(request[0],
                                            "num_prompt_tokens", -1),
                                    int(_fill))
