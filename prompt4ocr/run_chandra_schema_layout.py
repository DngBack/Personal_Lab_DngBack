...
def _ascii_fold(s: str) -> str:
    """Lowercase ASCII-ish key for fuzzy header match (drops accents)."""
    import unicodedata

    s = unicodedata.normalize("NFKD", s.lower())
    return "".join(ch for ch in s if "a" <= ch <= "z" or ch.isdigit())


def _header_matches_cash_list(inner_text: str) -> bool:
    """Chandra often OCRs ``KẾ`` / ``MẬT`` wrong; match by keywords."""
    t = _ascii_fold(inner_text)
    return ("bang" in t and "tienmat" in t) or (
        "bang" in t and "cash" in t and "list" in t
    )


def _header_matches_phan_ngan_hang(inner_text: str) -> bool:
    t = _ascii_fold(inner_text)
    return "phandanh" in t and "chonganhang" in t
...
def find_anchors(
    ...
    for sec_name in (
        ...
        for h in headers:
            ...
            ok = bool(hkey) and (sec_key in hkey or hkey in sec_key)
            if sec_name == "BẢNG KÊ TIỀN MẶT (CASH LIST)" and not ok:
                ok = _header_matches_cash_list(h["inner_text"])
            if sec_name == "PHẦN DÀNH CHO NGÂN HÀNG" and not ok:
                ok = _header_matches_phan_ngan_hang(h["inner_text"])
            if ok:
                candidates.append(h)

        match: dict[str, Any] | None = None
        if candidates:
            if sec_name == "PHẦN DÀNH CHO NGÂN HÀNG":
                match = max(
                    candidates,
                    key=lambda b: (b["bbox_norm"][1] + b["bbox_norm"][3]) / 2.0,
                )
            else:
                match = candidates[0]
...
# Anchors whose ``dy`` tracks the **top** of the form. Large tables / bank
# headers often sit on a different vertical scale than the template; mixing
# them into a plain median drags the whole overlay off (e.g. dy ≈ -27 pt).
PRIMARY_OFFSET_SCHEMA_NAMES = frozenset(
    {
        "Logo",
        "GIẤY GỬI TIỀN TIẾT KIỆM",
        "THÔNG TIN YÊU CẦU CỦA KHÁCH HÀNG",
...
def median_offset_robust(pairs: list[dict[str, Any]]) -> tuple[float, float, str]:
    """Median (dx, dy) with outlier-resistant strategy.

    Prefer ``Logo`` + title + customer header + cash-list header when at least
    two of those anchors exist; otherwise fall back to trimmed median (drop the
    single most extreme ``dy`` at top and bottom) if enough pairs; else plain
    ``median_offset``.
    """
    if not pairs:
        return 0.0, 0.0, "none"

    primary = [p for p in pairs if p["schema_name"] in PRIMARY_OFFSET_SCHEMA_NAMES]
    if len(primary) >= 2:
        return (*median_offset(primary), "primary")

    if len(pairs) >= 4:
        by_dy = sorted(pairs, key=lambda p: p["dy_pt"])
        trimmed = by_dy[1:-1]
        return (*median_offset(trimmed), "trimmed")

    return (*median_offset(pairs), "all")
...
def run() -> int:
    ...
    for src_path in inputs_list:
        for unit_name, image, page_pt in load_units(src_path, args.pdf_dpi):
            ...
            dx_pt, dy_pt, off_mode = median_offset_robust(anchors)
            ...
            print(
                f"[align] {unit_name}: dx={dx_pt:+.2f} dy={dy_pt:+.2f} pt "
                f"(mode={off_mode}, {len(anchors)} anchors)"
            )
            ...
            json_path.write_text(
                json.dumps({
                    ...
                    "alignment_offset_pt": {"dx": dx_pt, "dy": dy_pt, "mode": off_mode},
