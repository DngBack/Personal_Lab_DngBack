...
from prompt.baseline import build_baseline_prompt
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for baseline inference script."""
    parser = argparse.ArgumentParser(
        description="Run baseline OCR extraction prompt with vLLM."
    )
...
def run_baseline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    prompt = build_baseline_prompt(
        ...
        ocr_input_json=as_pretty_json(input_ocr),
    )
...
def main() -> None:
    """CLI entrypoint for baseline experiment."""
    args = parse_args()
    result = run_baseline(args)

    output_path = Path(args.output_dir) / f"baseline_{timestamp_slug()}.json"
    dump_json(result, output_path)
    print(f"Saved baseline result to: {output_path}")
